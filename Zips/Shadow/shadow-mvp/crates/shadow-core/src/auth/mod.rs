...
impl<'a> UserStore<'a> {
    ...
    pub fn create_user(
        ...
        self.conn.execute(
            "INSERT INTO user
                (id, name, role, created_at, settings_json, password_hash, email)
             VALUES (?1,?2,?3,?4,'{}',?5,?6)",
            rusqlite::params![id, username, role, now_unix(), hash, email],
        )?;
        audit(
            self.conn,
            actor,
            "user.create",
            &id,
            serde_json::json!({"name": username, "role": role}),
        )?;
    ...
    pub fn create_admin_replace_default(
        ...
        let user = self.create_user("system", username, password, "admin", email)?;
    ...
    pub fn get_user(&self, user_id: &str) -> Result<Option<User>, ShadowError> {
        let row = self
            .conn
            .query_row(
                "SELECT id, name, role, email, must_change_password, created_at
    ...
    /// Prüft Zugangsdaten. Berücksichtigt Kill-Switch und
    /// must_change_password. Loggt Erfolg/Misserfolg ins Audit-Log.
...
mod tests {
    ...
    fn create_and_delete_user() {
        ...
        let u = us.create_user("a", "bob", "bobpass", "user", None).unwrap();
