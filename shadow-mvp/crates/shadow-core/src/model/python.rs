...
impl ModelAdapter for PythonAdapter {
    ...
    fn health(&self) -> HealthStatus {
        match &self.io {
            Some(io) if self.loaded && io.child.try_wait().ok().flatten().is_none() => {
                HealthStatus::Ok
            }
            _ => HealthStatus::Down,
        }
...
mod tests {
    ...
    fn echo_cmd() -> Vec<String> {
        ...
        vec![
            "python3".into(),
            "-c".into(),
            format!("import sys; sys.path.insert(0, {py!r}); from shadow_ai.server import main; main()"),
        ]
    }
    ...
    fn end_to_end_echo() {
        ...
        let res = a.stream(req("Hallo Rust"), Box::new(|ev| {
